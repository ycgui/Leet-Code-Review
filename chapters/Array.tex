...
\section{Subarray Sum Equals K (M)}
...
\section{Continuous Subarray Sum (M)}
...
\section{Interval List Intersections (Facebook 2019.6.28)}
...
class Solution(object):
    def intervalIntersection(self, A, B):
        ...
        while i < len(A) and j < len(B):
            # lowerbound is the largest start point
            # upperbound is the smallest end point
            low = max(A[i][0], B[j][0])
            high = min(A[i][1], B[j][1])
            # if low > high, the no intersection
            if low <= high:
                res.append([low, high])
            # check next intersection by removing the smaller endpoint
            if A[i][1] < B[j][1]:
                i += 1
            else:
                j += 1
...
\begin{lstlisting}
# Clockwise 90deg rotation: Vertical flip + Transpose (Off-diagonal flip)
class Solution(object):
...
# Counter-clockwise 90deg rotation: Horizontal flip + Transpose (Off-diagonal flip)
class Solution(object):
    def rotate(self, matrix):
        """
        :type matrix: List[List[int]]
